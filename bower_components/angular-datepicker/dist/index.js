...
Module.directive('datePicker', ['datePickerConfig', '$timeout', function datePickerDirective(datePickerConfig, $timeout) {
    ...
    link: function (scope, element, attrs, ngModel) {
      ...
      function showPicker() {
        ...
        if (position === 'absolute') {
          ...
          if((offsetLeft + 280) > $(window).width())
            toRight = true;
          container = angular.element('<div date-picker-wrapper></div>');
          element[0].parentElement.insertBefore(container[0], element[0]);
          if(!toRight)
            picker.css({ left: 0, right: 'auto' });
          else
            picker.css({ left: 'auto', right: 0 });
